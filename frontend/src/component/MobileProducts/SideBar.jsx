...
function SideBar() {
    ...
    const passesPriceFilter = price >= filters.priceRange.min && price <= filters.priceRange.max;
    const passesBrandFilter = filters.brands.size === 0 || filters.brands.has(brand);
    // Check if rating is equal to 3 or 4
    const passesRatingFilter = filters.rating.size === 0 || 
            ...
            <div className="flex mt-4 ml-[90px]">
                <div className="h-[1000px] bg-gray-300 w-[300px]">
                    <div className='flex'>
                        ...
                        <button className='text-xs text-[#2874f0] mt-4 ml-32' onClick={clearAllFilters}>CLEAR ALL</button>
                    </div>
                    <hr />
                    <div className="mt-4 w-full">
                        ...
                        <br />
                        <p className="ml-4 mt-2">{filters.priceRange.min} - {filters.priceRange.max}</p>
                        <input
                    ...
                    <div>
                        ...
                        {Array.from(new Set(productList
                            .filter(item => item.brand?.toLowerCase().includes(filters.brand?.toLowerCase())) // Filter brands based on input
                            .map(item => item.brand)))
                            .map((brand, index) => (
                                <div className='ml-4 mt-1' key={index}>
                                    ...
                                    />
                                    <label className='ml-2' htmlFor={`brand_${index}`}>{brand}</label>
                                </div>
                            ))}
                    </div>
                    ...
                    <div>
                        ...
                        <div className='ml-4'>
                            ...
                            />
                            <label className='ml-2' htmlFor={`rating_4`}> 4+</label>
                        </div>
                        ...
                        <div className='ml-4'>
                            ...
                            />
                            <label className='ml-2' htmlFor={`rating_3`}> 3+</label>
                        </div>
                    </div>
                </div>
                ...
                <div className="ml-2">
                    {filteredProducts.map((item, i) => (
                        <div key={i} className="bg-gray-100 h-[300px] w-[1400px] ml-2 flex border border-gray-300">
                            <div className="mt-6 pt-4">
                                <img className="h-[180px] mt-6 w-[240px]" src={item?.image} alt="Product" />
                            </div>
