...
const dispatch = useDispatch()
    ...
    const handleOnclick = (id)=>{
     const removeId = orderList.find((order) => order._id === id);
...
<div>
{
    orderList?.map((item)=>(
        <div className='h-[150px] w-[600px] bg-gray-200 border border-white m-2 flex justify-between'>
            ...
            <h1 className=' mt-2 ml-4'>Shipping Address :  <span>{item.shippingAddress}</span>, <span>{item.city}</span>, <span>{item.zip}</span> , <span>{item.country}</span> </h1>
            <h1 className=' mt-2 ml-4'>Phone : <span>{item.phone}</span></h1>
             </div>
